...
哈希冲突：不同对象的 hashCode 可能相同，但 hashCode 不同的对象一定不相等，所以使用 hashCode 可以起到快速初次判断对象是否相等的作用。

# 内存结构

![](https://img-blog.csdnimg.cn/cb59671dff1f4588b5d845f60874013b.png)

JVM 的运行时内存结构主要包括五个部分：程序计数器、Java 虚拟机栈、本地方法栈、堆和方法区。其中程序计数器、虚拟机栈和本地方法栈是线程私有的；堆和方法区是线程共享的。堆主要用于存储对象实例，而方法区在 JDK8 之后由 Metaspace 实现，用于存储类信息、运行时常量池和静态变量等数据。
...
在每个volatile读操作后插入LoadLoad屏障 + LoadStore屏障，防止后续的读写操作被重排序到 volatile 读之前；在每个volatile写操作的前面插入一个StoreStore屏障，后面插入一个SotreLoad屏障，防止后续的读写操作被重排序到 volatile 读之前。
...
## 对象的创建过程
1. 类加载检查
2. 分配内存：指针碰撞（适用于Serial GC或ParNew）或空闲列表（适用于 CMS）
...
1. 对象头，两部分组成：存储自身运行时数据如哈希码、GC分代年龄、锁状态、偏向锁信息；指向类的类型指针
2. 实例数据，真正存储有效信息的部分
3. 对齐填充，起占位作用
...
# Java IO
1. BIO(Block IO)：同步阻塞IO，使用方便，但并发处理能力低
2. NIO(New IO)：同步非阻塞IO，适用于连接数目多且连接比较短（轻操作）的架构
3. AIO(Async IO)：异步非阻塞IO，适用于连接数目多且连接比较长（重操作）的架构
...
- 主要特点：
  1. **面向缓冲区（Buffer）**
     - 所有数据读写都通过 Buffer（内存块）完成，类比成载着货物的火车
  2. **面向通道（Channel）**
     - Channel 可以 **双向传输数据**（读/写），类比成铁路
  3. **非阻塞（Non-blocking）**
     - 可以在单线程中处理多个连接，不会被单个 IO 阻塞
  4. **选择器（Selector）**
     - 多路复用机制，可监听多个 Channel 的事件（比如：连接打开，数据到达），如果由事件发生，则获取事件并对每个事件进行相应的响应处理。
...
- 偏向锁，是一种针对单线程访问同步代码场景的优化机制。当一个线程第一次获取锁时，JVM 会在对象头的 Mark Word 中记录该线程 ID，并将锁标记为偏向状态。之后该线程再次进入同步代码块时，只需要检查对象头中的线程 ID 是否为当前线程，如果是则可以直接进入，而不需要执行 CAS 操作，从而减少加锁和解锁的开销。
- 轻量级锁，通过用CAS修改Mark Word操作和**自旋**来解决加锁问题，避免线程阻塞和唤醒而影响性能。
- 重量级锁，将除了拥有锁的线程以外的线程都阻塞。

锁升级的过程：

当有线程访问同步块时，无锁升级为偏向锁；当有锁竞争时，升级为轻量级锁；当自旋十次失败，升级为重量级锁。

4. 公平锁，非公平锁：
- 公平锁，每个线程获取锁的顺序是按照线程访问锁的先后顺序获取的。
- 非公平锁，线程获取锁时并不会遵循先来先得的规则，可以插队，当后到的线程请求锁时，该锁恰好被释放，则该锁被后到的线程拥有。非公平锁并不是随机分配锁，而是允许新线程在获取锁时直接通过 CAS 尝试抢占锁，该机制只是“允许插队尝试”，如果没有抢到锁，线程仍然需要进入队列排队等待。
